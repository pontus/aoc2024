...
func order(update []int, pairs []pairing) {
	fmt.Printf("Ordering %v", update)
			...
			for _, p := range pairs {
				...
				for x, v := range update {
					if v == p.after {
						// The pairing we want to check
						if x < i {
							fmt.Printf("Need to swap %v and %v in %v because of %v\n", i, x, update, p)

							update[x] = p.before
							update[i] = p.after
							u = p.after
							fmt.Printf("After swap of  %v and %v in %v because of %v\n", i, x, update, p)
							changed = true
							break
						}
					}
