...
func sizer(cover, factor int) int {
	if factor > cover {
		return factor
	}
	return sizer(cover, factor*10)
}
...
func canBe(result int, vals []int, locked []int) bool {
	if len(locked) == len(vals)-1 {
		...
		for i, op := range locked {
			...
			case 1:
				count = count + vals[i+1]
			case 2:
				count = count*sizer(vals[i+1], 10) + vals[i+1]
			}
		}
	...
	if canBe(result, vals, tryAdd) {
		return true
	}

	tryConcat := append(locked, 2)
	return canBe(result, vals, tryConcat)
